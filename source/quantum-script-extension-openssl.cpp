...
#include "quantum-script-extension-openssl.src"
		...
		namespace Extension {
			namespace OpenSSL {
				...
				void initExecutive(Executive *executive, void *extensionId) {
					...
					executive->compileStringX(extensionOpenSSLSource);
				};
					...
					if (bio != nullptr) {
						...
						if (rsa != nullptr) {
							...
							if (RSA_private_encrypt(
								...
								XYO::Buffer outputData;
								Crypt::passwordEncrypt(randomKey, 64, data, dataSize, outputData);
				...
				bool privateDecrypt(const uint8_t *privateKey, size_t privateKeySize, const uint8_t *data, size_t dataSize, XYO::Buffer &output) {
					...
					if (bio != nullptr) {
						...
						if (rsa != nullptr) {
							...
							return Crypt::passwordDecrypt(randomKey.buffer, 64, &data[rsaSize], dataSize - rsaSize, output);
						};
					};
...
extern "C" QUANTUM_SCRIPT_EXTENSION_OPENSSL_EXPORT void quantumScriptExtension(Quantum::Script::Executive *executive, void *extensionId) {
	Quantum::Script::Extension::OpenSSL::initExecutive(executive, extensionId);
};
#endif
